...
//  TikiBird
//
//  Created by Drew Westcott on 12/07/2016.
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //Obstacles
    var tikis = [SKNode]()
    let heightBetweenObstacles: CGFloat = 907
    let timeBetweenObstacles = 3.0
    let bottomTikiMaxYPos = 234 //308
    let bottomTikiMinYPos = 380 //-120
    ...
    func initSetup() {
        ...
        let gameOverURL = Bundle.main.urlForResource("game-over", withExtension: "wav")!
        do {
            gameOverSound = try AVAudioPlayer(contentsOf: gameOverURL)
            print("DW: sound loaded")
        } catch {
            print("DW: Music not played")
        }
        gameOverSound.numberOfLoops = 0
        gameOverSound.prepareToPlay()
        
        let tapSoundURL = Bundle.main.urlForResource("tap", withExtension: "wav")!
        do {
    ...
    func startGame() {
        for sprite in groundPieces {
            //added withKey in order to pause action when collision occurs.
            sprite.run(moveObstacleForeverAction, withKey: "gameStarted")
        }
